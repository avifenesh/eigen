...
// runMarketplaceCmd implements `eigen marketplace <add|list|remove|update>`.
func runMarketplaceCmd(args []string) {
	...
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: eigen marketplace <add|list|remove|update> …")
		os.Exit(2)
	}
	switch args[0] {
	case "add":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: eigen marketplace add <owner/repo[/subdir][@ref] | url>")
			os.Exit(2)
		}
	...
	case "list":
		...
		if len(markets) == 0 {
			fmt.Println("no marketplaces added (eigen marketplace add <owner/repo>)")
			return
		}
		for _, m := range markets {
			when := m.Added.Format("2006-01-02")
			fmt.Printf("%-20s %s  (added %s)\n", m.Name, m.Source, when)
		}
	case "remove", "rm":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: eigen marketplace remove <name>")
			os.Exit(2)
		}
		...
		fmt.Printf("removed marketplace %q (installed plugins are unaffected)\n", args[1])
	case "update", "refresh":
	...
	default:
		fmt.Fprintf(os.Stderr, "unknown marketplace subcommand %q (want: add | list | remove | update)\n", args[0])
		os.Exit(2)
	}
}

// runPluginCmd implements `eigen plugin <install|list|remove|enable|disable>`.
func runPluginCmd(args []string, provider, model string) {
	...
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: eigen plugin <install|list|remove|enable|disable> …")
		os.Exit(2)
	}
	switch args[0] {
	case "install", "add":
	...
	case "remove", "rm", "uninstall":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: eigen plugin remove <name>")
			os.Exit(2)
		}
		...
		fmt.Printf("removed plugin %q (skills, mcp servers, hooks, and bundle)\n", args[1])
	case "enable", "disable":
		if len(args) < 2 {
	...
	default:
		fmt.Fprintf(os.Stderr, "unknown plugin subcommand %q (want: install | list | remove | enable | disable)\n", args[0])
		os.Exit(2)
	}
}
