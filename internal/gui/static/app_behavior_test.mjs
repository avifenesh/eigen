...
function makeContext() {
  ...
  const ctx = {
    ...
    fetch: async (path, opts = {}) => {
      ...
      else if (String(path).includes('/memory')) data = {dir: '/tmp/project', memory: '# notes\n', bans: [{title: 'no push', rule: 'never push'}], notes: ['a note'], files: ['MEMORY.md', 'bans.md']};
      else if (String(path).includes('/skills')) {
        if (String(path).includes('name=')) data = {name: 'curator', body: '# curator\nskill body'};
        else data = [{name: 'curator', description: 'Skill curation'}, {name: 'deslop', description: 'Clean slop'}];
      }
...
// Memory feature loads the project memory workspace.
await new Promise(r => setTimeout(r, 0));
assert.match(document.getElementById('feature-stage').innerHTML, /MEMORY\.md|Bans|Ad-hoc notes/i, 'memory surface renders the project workspace cells');
ctx.setFeature('skills');
await new Promise(r => setTimeout(r, 0));
assert.match(document.getElementById('feature-stage').innerHTML, /Skills|curator|deslop/i, 'skills surface renders discovered skills');
ctx.setFeature('chat');
