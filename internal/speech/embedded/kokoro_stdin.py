#!/usr/bin/env python3
"""Read stdin aloud with Kokoro ONNX and aplay (eigen TTS backend).

Vendored from the author's codex-desktop-linux read-aloud backend and
renamed to EIGEN_KOKORO_* env vars — eigen is independent; no external
checkout is required at runtime. Requires: python3 with kokoro_onnx +
numpy, aplay (alsa-utils), and the Kokoro model/voices files.
"""
...
def split_for_streaming(text: str) -> list[str]:
    first_chars = max(40, env_int("EIGEN_KOKORO_FIRST_CHARS", 90))
    chunk_chars = max(80, env_int("EIGEN_KOKORO_CHUNK_CHARS", 180))
    cleaned = text.strip()
...
def main() -> int:
    threads = str(env_int("EIGEN_KOKORO_THREADS", 4))
    ...
    model = os.environ["EIGEN_KOKORO_MODEL"]
    voices = os.environ["EIGEN_KOKORO_VOICES"]
    voice = os.environ.get("EIGEN_KOKORO_VOICE", "bm_george")
    speed = min(1.4, max(0.7, env_float("EIGEN_KOKORO_SPEED", 1.05)))
    lang = os.environ.get("EIGEN_KOKORO_LANG", "en-us")
