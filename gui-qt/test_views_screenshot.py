    ...
    def visit(item):
        ...
        if item.property("qaIsAppTag") is True:
            ...
            if visible and width > 0 and height > 0:
                ...
                if item.property("qaTextFits") is not True or horizontal_padding < 35.5 or vertical_padding < 9.5:
                    failures.append(
    ...
    def visit(item):
        ...
        if any(item.property(marker) is True for marker in markers):
            ...
            if visible and width > 0 and height > 0:
                horizontal_padding = float(item.property("qaHorizontalPadding") or 0)
                vertical_padding = float(item.property("qaVerticalPadding") or 0)
                if item.property("qaTextFits") is not True or horizontal_padding < 27.5 or vertical_padding < 7.5:
                    failures.append(
                        f"{item.objectName() or '<unnamed>'} "
                        f"fits={item.property('qaTextFits')} "
                        f"padding={horizontal_padding:.1f}x{vertical_padding:.1f} "
...
def main():
    ...
    def assert_home_tags(_view, root):
        for object_name in (
            ...
            if tag.property("qaTextFits") is not True or float(tag.property("qaHorizontalPadding") or 0) < 35.5 or float(tag.property("qaVerticalPadding") or 0) < 9.5:
    ...
    def open_chat_terminal_dock(_view, root):
        ...
        if status_tag.property("qaTextFits") is not True or float(status_tag.property("qaHorizontalPadding") or 0) < 35.5 or float(status_tag.property("qaVerticalPadding") or 0) < 9.5:
            raise AssertionError(
