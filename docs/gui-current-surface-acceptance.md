...
- native/browser desktop GUI (`internal/gui`): local-only server, premium multi-surface desktop workspace (chat, changes, tools, shells, approvals, memory, plugins, config), interactive config, approval, shell, memory, compact, add-dir, and allow-tool controls, static shell/API contracts, health/profile/session API seams, service validation, stream shutdown, browser smoke;
...
| --- | --- |
| Premium desktop app shell exists | Native/browser GUI shell is local-only and smoke-tested with a multi-surface desktop workspace plus interactive controls for config settings, approvals, shell kill/detach, memory goal/compact/add-dir, allow-tool turns, profile/system modals, and feature navigation (`internal/gui:TestServeRejectsNonLocalBind`, `TestHandlerStaticAndAPIContracts`, `node internal/gui/static/app_behavior_test.mjs`, `scripts/gui-smoke.sh`); app shell has premium visual contracts/goldens (`internal/app:TestAppPremiumShellVisualContract`, `TestAppLiveSessionsPluginsGoldenSnapshotTokens`); release app shell screenshot artifact exists (`docs/artifacts/gui/release-app-shell.png`). |
| Includes all Eigen GUI features | Code-derived parity inventory tests enumerate app pages directly from `internal/app.pages`, TUI actions directly from `internal/tui.actionRegistry`, and right-panel tabs from `internal/tui` tab constants, then require journey evidence for every entry (`internal/app:TestAppCodeDerivedFeatureInventoryHasJourneyEvidence`, `internal/tui:TestTUICodeDerivedFeatureInventoryHasJourneyEvidence`). Feature parity docs add native GUI, mutating-page, and keyboard/accessibility mappings (`docs/gui-feature-parity.md`, `docs/gui-mutating-pages-evidence.md`, `docs/gui-accessibility-keyboard-audit.md`). No shipped GUI feature is recorded as missing. |
...
| High UI/UX quality | Real running-Wails artifact bundle captures the actual native app window launched with `go run -tags 'wails production webkit2_41' . --instance dev gui` (`docs/artifacts/gui/real-wails/manifest.json`, `docs:TestGUIRealWailsArtifactBundle`). Native GUI visual artifact bundle also captures all eight desktop surfaces as rendered 1440px screenshots (`docs/artifacts/gui/native/manifest.json`, `docs:TestGUINativeVisualArtifactBundle`). Objective UI metrics tests assert rendered app pages and TUI tabs fit terminal bounds, include product/navigation tokens, and reject placeholder/debug copy (`internal/app:TestAppObjectiveUIQualityMetrics`, `internal/tui:TestTUIObjectiveUIQualityMetrics`). Premium shell visual contracts, all-page goldens, richer live/sessions/plugins goldens, TUI composer/right-panel goldens, screenshot artifacts, and keyboard parity evidence are recorded in `docs/gui-parity-evidence.md`. |
