...
- native/browser desktop GUI (`internal/gui`): local-only server, premium multi-surface desktop workspace (chat, changes, tools, shells, approvals, memory, plugins, config), static shell/API contracts, health/profile/session API seams, service validation, stream shutdown, browser smoke;
