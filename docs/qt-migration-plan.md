...
| C | **Flip + delete** (old P6+P7) | Flip `eigen-gui.desktop` Exec to the Qt launcher. No soak window — he IS the soak; fallback is a one-line `Exec=` edit for the first days (don't delete `frontend/` the same week as the flip). Keep `eigen-gui-legacy` building for the DON'T-PORT tail; delete `frontend/` + wails tags + gui-phase workflow when he notices he hasn't launched legacy in a while | One primary GUI; both gates green; legacy binary is a museum piece until it isn't built at all | **FLIPPED** — `gui-qt/install-desktop.sh` writes `~/.local/bin/eigen-qt`, `eigen-qt.desktop`, and the primary `eigen-gui.desktop` entry. Keep `eigen-gui-legacy` available as fallback until the legacy frontend is deliberately removed. |
