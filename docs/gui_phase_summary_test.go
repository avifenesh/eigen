...
func TestGUIPhaseSummaryIsMachineReadable(t *testing.T) {
	...
	var s struct {
		...
		GUIBehaviorCheck        string   `json:"gui_behavior_check"`
	...
	if s.GUIBehaviorCheck != "node internal/gui/static/app_behavior_test.mjs" {
		t.Fatalf("GUI behavior check = %q", s.GUIBehaviorCheck)
	}
