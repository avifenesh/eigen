...
func TestGUIPhaseSummaryIsMachineReadable(t *testing.T) {
	...
	var s struct {
		...
		RemainingBeforeFutureScope []string `json:"remaining_before_full_goal"`
	}
	if err := json.Unmarshal(b, &s); err != nil {
		t.Fatal(err)
	}
	if s.Status != "current_surface_milestone_complete_persistent_goal_continues_for_future_scope" {
	...
	if len(s.RemainingBeforeFutureScope) == 0 {
		t.Fatal("future-scope work must stay explicit")
	}
}
