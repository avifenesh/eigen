...
| --- | --- | --- |
| Native/browser desktop GUI shell | Daemon-backed GUI stays local-only, serves a premium multi-surface desktop workspace (chat, changes, tools, shells, approvals, memory, plugins, config), includes interactive controls for config settings, approvals, shell kill/detach, memory goal/compact/add-dir, allow-tool turns, profile/system modals, and feature navigation, exposes health/session/profile APIs, validates service inputs before daemon calls, and stops event streaming cleanly. | `internal/gui:TestServeRejectsNonLocalBind`, `internal/gui:TestHandlerStaticAndAPIContracts`, `internal/gui:TestServiceValidationErrors`, `internal/gui:TestStreamJSONLinesStopsOnContextOrClosedEvents`, `node internal/gui/static/app_behavior_test.mjs`, `scripts/gui-smoke.sh` |
...
| Code-derived app feature parity | Every app page declared in `internal/app.pages` must have product copy and journey/golden evidence; adding a page without evidence fails tests. | `internal/app:TestAppCodeDerivedFeatureInventoryHasJourneyEvidence` |
| Code-derived TUI feature parity | Every TUI chrome action declared in `internal/tui.actionRegistry` and every right-panel tab constant must have journey evidence; adding an action/tab without evidence fails tests. | `internal/tui:TestTUICodeDerivedFeatureInventoryHasJourneyEvidence` |
| Real Wails visual artifact bundle | The actual running Wails app was launched in an isolated X11 workspace (`go run -tags 'wails production webkit2_41' . --instance dev gui`) and captured as 1440x1000 PNGs for Chat, Changes, Tools, Shells, Approvals, and Memory with provenance, dimensions, file size, and color-variance checks; the real-Wails layout fix hides the overview on feature surfaces so direct controls are visible in the window. | `docs/artifacts/gui/real-wails/manifest.json`, `docs:TestGUIRealWailsArtifactBundle` |
| Native GUI visual artifact bundle | The native/browser GUI workspace has 1440px rendered screenshots for Chat, Changes, Tools, Shells, Approvals, Memory, Plugins, and Config, with manifest-required text/actions and non-trivial PNG sizes. | `docs/artifacts/gui/native/manifest.json`, `docs:TestGUINativeVisualArtifactBundle` |
| Objective UI quality metrics | Rendered app pages and chat TUI right-panel tabs must fit terminal bounds, expose product/navigation tokens, and avoid placeholder/debug copy. | `internal/app:TestAppObjectiveUIQualityMetrics`, `internal/tui:TestTUIObjectiveUIQualityMetrics` |
