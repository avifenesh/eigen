...
- **Key symbols:**
  ...
  - `Open(projectDir)` / `OpenGlobal()` — return the project / cross-project store; each runs `migrateFlat`. **Project scope is keyed by the CANONICAL project root** (git main-worktree root via `canonicalProjectDir`, gitroot.go), so a worktree, subdir, or `..`-rooted session all map to ONE scope instead of fragmenting; non-git dirs fall back to the abs-path hash (unchanged).
  - Scope movement/merge/forget (gitroot.go, merge.go, forget.go): `MoveNote(src,dst,note)` relocates a fact between ANY two stores (promote project→global, demote global→project, OR project→project; records in dest + tombstones the source for the next consolidation); `MergeFrom`/`MergeByKey`/`ReconcileScopes` fold a fragmented scope into its canonical one (append under a provenance header, copy ad-hoc+rollouts, archive the source — never delete); `PruneEvidence`/`RetireAdHocNotes` are the `mem_forget` tier (retire folded ad-hoc notes out of Phase 2 input; archive rollout summaries past the recency/age cap).
  ...
  - Helpers: `baseDir()`, `key(abs)` (base name + sha1[:8]), `slugify`, `scopeLabel`.
- **Depends on:** stdlib only (crypto/sha1, os, filepath, regexp, sort, time) + sibling files (`Redact` from redact.go, `canonicalProjectDir` from gitroot.go, `OpenIndex`/`Enqueue`/job consts from index.go/pipeline.go).

### internal/memory/gitroot.go
- **Role:** Canonical project-scope resolution — maps any dir inside a git repo to the repo's MAIN-worktree root so worktrees/subdirs/`..` share one memory scope (fixes scope fragmentation).
- **Key symbols:** `canonicalProjectDir(dir)` (cached via `canonCache sync.Map`) → `resolveProjectRoot` (prefers `git rev-parse --git-common-dir`'s parent — shared across all worktrees — then `--show-toplevel`, then the symlink-resolved abs path); `mainRootFromCommonDir`, `gitRootText` (timeout-bounded `git -C`), `gitRootTimeout` (3s).
- **Depends on:** stdlib (os/exec, path/filepath, sync, context, time).
- **Used by:** `Open` (memory.go), `ReconcileScopes` (merge.go).

### internal/memory/merge.go
- **Role:** Scope merge — heal pre-existing fragmentation by folding an orphan scope into the canonical one, non-destructively.
- **Key symbols:** `MergeResult` struct; `(*Store).MergeFrom(src, when)` (appends src `MEMORY.md` under a `## Merged from scope <key>` header, copies ad-hoc+rollouts collision-safe, archives the source dir to `<key>.merged-<ts>`, enqueues consolidation); `MergeByKey(srcKey, dstKey, when)` (manual/GUI + deleted-worktree orphans); `ReconcileScopes(knownDirs, when)` (auto-heal: merge each dir whose legacy cwd-key ≠ canonical git-root key AND whose legacy scope still exists); `copyMarkdown`, `archiveDir`.
- **Depends on:** stdlib + sibling `Store`.
- **Used by:** `daemon.go` (`reconcileMemoryScopes` on startup), `internal/gui/memory.go` (`MergeMemoryScope`).

### internal/memory/forget.go
- **Role:** The `mem_forget` tier — bound the append-only evidence tiers so a busy scope's disk/glob stays sane.
- **Key symbols:** `ForgetResult`; `(*Store).PruneEvidence(now)` → `pruneRollouts` (archive rollout summaries past `keepRecentRollouts`=80 or `rolloutMaxAge`=45d into `rollout_summaries/archive/`) + `pruneRetiredAdHoc` (archive folded ad-hoc notes older than `retiredMaxAge`=90d); `archiveFile`. AGE+COUNT based, not usage (the `usage_count`/`last_used` index columns aren't wired to a real consult signal yet).
- **Depends on:** stdlib + sibling `Store`.
- **Used by:** `Pipeline.RunQueued` (the `JobForget` case).
...
- **Key symbols:**
  - Job-kind consts (`JobStage1`/`JobConsolidate`/`JobSummary`/`JobForget`), `scopeJobKey`, chunking limits.
  ...
  - `RunQueued(ctx, maxJobs)` — drain this scope's consolidate/summary/**forget** jobs via `ClaimScope` + `Finish` (the `JobForget` case runs `Store.PruneEvidence`).
  - `MaybeConsolidate(ctx, force)` — rewrite `MEMORY.md` when over threshold (default 24 KB) or forced; builds `phase2Input()` (current memory + Stage1 raw memories + ad-hoc notes; now returns the **ad-hoc file paths** too), writes raw scratchpad, runs `consolidatePhase2`, rewrites, marks selected, then **`RetireAdHocNotes`** the folded ad-hoc files so they stop re-feeding future Phase 2. `Stage1Sessions` enqueues `JobForget` alongside consolidate/summary.
...
- **Key symbols:** `summarizePrompt` + `Summarize(ctx, p, memory)` — distil `MEMORY.md` into the small injected `memory_summary.md` (refuses a summary not smaller than input); `maxSummarizeInput` (200k); `globalProfilePrompt` + `DistillGlobal(ctx, p, summaries, existingGlobal)` — extract project-independent user-profile bullets from many projects' summaries (deduped; the PROMOTE direction, project→global); `stationPrompt` + `DistillStation(ctx, p, digest, existingGlobal)` — reflect a WORKING-STATION digest (calendar/mail/projects/crons/health) into durable life-awareness notes for global memory; `demotePrompt` + `ProposeDemotions(ctx, p, globalMemory, projectNames)` — the DEMOTE direction: find global notes that are really specific to one named project and return `[]Demotion{Project,Note}` (conservative — only confident, in-candidate-set proposals; `splitDemotion` parses the `PROJECT<TAB>note` lines).
- **Depends on:** `internal/llm`; reuses `dedupe`/`parseBullets` from dream.go.
- **Used by:** `Summarize` wired as `Pipeline.Summarize`; `DistillGlobal` + `DistillStation` called by `daemon.go` nightly dreamer + `main.go` `runDream`; `ProposeDemotions` called by `daemon.go:reclassifyGlobalDown` (applies each via `memory.MoveNote`). The station digest is built by `main.go:stationDigest` (Google calendar/mail via `internal/google`, project git state via `internal/feed`, crontab, `internal/syshealth`); routine input → no notes.
...
- **`internal/tool`** — `internal/tool/memory.go` exposes the memory tool to the agent via `MemoryStore`/`memoryReader`/`memorySearcher` interfaces (`Append`, `AddBan`, `ReadRelative`, `Search`) satisfied by `*memory.Store`; the `move` action (`moveNote`/`pickMoveScope` helpers) resolves each scope to a `*memory.Store` — "project"/"global" → the session stores, anything else → another project by on-disk key or readable name (`ListProjectStores`) — and calls `memory.MoveNote`, so a fact can move project→project, not only project↔global. (See `tool-actions.md`.)
...
- **Daemon (`daemon.go`)** — `reconcileMemoryScopes()` on startup folds fragmented per-cwd scopes into their canonical git-root scope (via `memory.ReconcileScopes` over persisted-session dirs). Nightly dreamer: opens global + per-project stores, builds pipelines, runs `dream.SynthesizeSkill`/`DistillGlobal`, then `reclassifyGlobalDown` (demote misfiled global notes into their project via `dream.ProposeDemotions` + `memory.MoveNote`), commits memory. (See `daemon.md`.)
