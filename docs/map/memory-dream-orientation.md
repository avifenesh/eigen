...
- **Key symbols:**
  ...
  - Scope movement/merge/forget (gitroot.go, merge.go, forget.go): `MoveNote(src,dst,note)` relocates a fact (promote project→global / demote global→project; tombstones the source for the next consolidation); `MergeFrom`/`MergeByKey`/`ReconcileScopes` fold a fragmented scope into its canonical one (append under a provenance header, copy ad-hoc+rollouts, archive the source — never delete); `PruneEvidence`/`RetireAdHocNotes` are the `mem_forget` tier (retire folded ad-hoc notes out of Phase 2 input; archive rollout summaries past the recency/age cap).
...
- **`internal/tool`** — `internal/tool/memory.go` exposes the memory tool to the agent via `MemoryStore`/`memoryReader`/`memorySearcher` interfaces (`Append`, `AddBan`, `ReadRelative`, `Search`) satisfied by `*memory.Store`; the `move` action (`moveNote` helper) type-asserts both stores to `*memory.Store` and calls `memory.MoveNote` to promote/demote a fact. (See `tool-actions.md`.)
