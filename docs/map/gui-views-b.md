...
- **Role:** The "superapp" integrations view (`connectors` route, rail "Connectors"). Sections: **native built-in cards** — Google (Calendar + Gmail; connect/disconnect/setup), Obsidian (vault notes; auto-detected path), Revuto (PR-reviewer; reviewer count) ; configured remote connectors with OAuth status (connect/disconnect/remove); a **curated directory grid** (one-click add+authorize for Notion/Linear/Asana/…); and local stdio MCP servers with an **add/edit form** (name/command/env + secret env → OS keychain, gated on `MCPSecretsAvailable`; 🔑 badge). Add a custom remote one by name + URL. Refreshes on the `eigen:connector` event. Built-in cards read `ObsidianStatus`/`RevutoStatus`.
