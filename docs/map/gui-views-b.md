...
- **Role:** The "superapp" integrations view (`connectors` route, rail "Connectors"). Sections: a **native Google card** (Calendar + Gmail — connect/disconnect, or a setup hint when no Google Cloud client is present); configured connectors with OAuth status (connect/disconnect/remove); a **curated directory grid** (one-click add+authorize for Notion/Linear/Asana/…); and local stdio MCP servers with an **add/edit form** (name/command/env + secret env → OS keychain, gated on `MCPSecretsAvailable`; secret count shown as a 🔑 badge). Add a custom remote one by name + URL. Refreshes on the `eigen:connector` event when a background flow finishes.
