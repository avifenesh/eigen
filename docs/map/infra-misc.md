...
- **Key symbols:**
  - `Config` (struct) — every optional setting: provider/model (one user-facing model ref; provider is shadow metadata), perm, input_mode (steer|queue), effort, theme, nerd_font, max_tokens, tts_cmd, notify_cmd, judge_model (pins the goal/claim judge — read into Agent.JudgeModel, beats the cheap-judge type ladder), dream_model (pins background dreaming — empty = sonnet-first dream ladder, off the cheap GLM quota), dream_batch (route nightly dream Stage1 through the provider async batch API, ~50% off — Anthropic only, default off), telegram_token/telegram_allow, skills_dirs, route/route_providers, observe (`*bool`, default-on), dream_on_idle/idle_minutes, front_window_min/stall_idle_min, local_background, daemon_timeout.
