...
| telegram-mcp | The MCP client over stdio + remote Streamable HTTP (wraps server tools as native Eigen tools with lazy connect, allowlists, a typed mcp.json editor, and built-in helper auto-detection); `internal/connector` adds OAuth 2.1 + PKCE for remote connectors (Google WS, Slack, Notion) with keychain-stored tokens; plus the Telegram bridge exposing live daemon sessions on your phone with streaming replies and inline approval buttons. | 16 | [telegram-mcp](telegram-mcp.md) |
