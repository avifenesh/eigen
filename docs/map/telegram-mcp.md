...
> minimal Model Context Protocol stdio client: it spawns MCP server subprocesses,
> speaks JSON-RPC 2.0 over newline-delimited stdio, lists each server's tools, and
> wraps them as native Eigen `tool.Definition`s (named `<server>_<tool>`) with
> progressive-disclosure grouping, allowlist filtering, schema slimming, lazy
> per-session connections, and built-in auto-detection of Eigen's bundled
> workspace / computer-use / chrome-bridge helpers. **`internal/telegram`** is a
...
- **Role:** The low-level MCP stdio JSON-RPC client — process spawn, handshake, request/response plumbing, tool listing and invocation.
- **Key symbols:**
  ...
  - `Client` — a connected MCP session: JSON encoder, pending-call map keyed by request id, `closeFn`, cached `instructions` / `serverName`.
  ...
  - `(*Client).readLoop` — scans newline-delimited responses (8 MiB buffer), routes by id to the waiting channel, fails in-flight calls on stream close.
  ...
  - `(*Client).CallToolRich` — `tools/call`; returns concatenated text plus base64-decoded image blocks (capped at `maxMCPImages`=4 / `maxMCPImageBytes`=4 MiB); maps `isError` to a Go error.
  - `(*Client).CallTool` — thin text-only wrapper over `CallToolRich` (drops images).
  - `(*Client).Instructions` / `(*Client).ServerName` — accessors for the server's self-description / reported name.
  - `(*Client).Close` — invokes `closeFn` (graceful subprocess teardown).
  - `ToolResult` — text + `[]llm.Image`, mirrors `tool.Result`.
- **Depends on:** `internal/llm` (for `llm.Image` in `ToolResult`).
- **Used by / entrypoint:** `load.go` (`Connect`, `ListTools`, `Instructions`, `CallToolRich` via `lazyClient`). `Connect`/`ListTools` reached from `LoadTools`. `CallTool` and `ServerName` are test-only (see Dead code).
...
- **Key symbols:**
  - `serverConfig` — one `mcp.json` entry: `Name`, `Command`, `Env`, `Description`, `Tools` allowlist, `ExcludeTools`, `Disabled`.
  - `mcpConfig` — `{ servers: [...] }`.
  - `Handle` — interface (`Close() error`) for a per-session MCP resource returned to callers.
  - `LoadTools(ctx, path)` — reads `mcp.json` (missing file is fine), applies `withBuiltinServers`, probes each enabled server once for schemas (then closes the probe), derives a Level-0 group "gist" (config description → server instructions → warning), and returns `[]tool.Definition` + `[]Handle` (lazy clients) + non-fatal `errs`.
  - `lazyClient` — owns a server started only on first tool invocation; `get` dials once under a mutex, `CallToolRich` proxies, `Close` is idempotent, `started()` is a test probe.
  - `newLazyClient` — constructs a `lazyClient` (copies command/env, wires `connect=Connect`).
...
- **Depends on:** `internal/tool` (`tool.Definition`, `tool.Result`); same-package `Connect`/`Client`/`withBuiltinServers`.
- **Used by / entrypoint:** `LoadTools` called from `build.go:223` and `main.go:773` (the agent's tool-layer assembly). Returned `Handle`s stored as `mcpClients` and closed at session end.
