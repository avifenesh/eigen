...
- **Key symbols:**
  ...
  - `send()` — `async`, guards `canSubmit && !sending`, awaits `onsend`; clears text + revokes the image batch + regrows ONLY when `onsend` doesn't return `false` (a failed RPC keeps the carefully-composed prompt). `sending` state disables a double-fire. `onkeydown` (Enter sends, Shift+Enter newlines, respects IME composing). **Mid-turn steering:** the keyboard/send path gates on `canSubmit` (text-or-images, online) NOT `canSend` (which also excludes `running`) — so Enter works WHILE a turn runs to steer it; the old `!running` gate here is what made mid-turn input silently no-op. The visible button still flips to Stop while running, so the two never conflict; the hint reads "Enter steers the running turn" mid-turn.
  ...
  - derived: `trimmed`, `canSubmit` (text-or-images, online — gates Enter/send so mid-turn steering works), `canSend` (`canSubmit && !running` — the visible Send button's enabled state), `hint`/`status` (char count / image count / shortcut help; "Enter steers the running turn" while running), `offlineReason`; `statusId` (`$props.id()`) wires the textarea's `aria-describedby` to the live status line.
