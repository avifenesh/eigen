...
> `harness`/`computer-use`/`workspace`/`chrome`, `models`, `dev`, `theme`,
> `version`), and decides whether to open the paged **app** shell, attach a TUI
> view to a daemon session, run headless print mode, or drive an in-process
> agent. The other files specialize a concern: `daemon.go` is the long-lived
> session host + its control surface + nightly dreaming + the Telegram
...
> desktop GUI; `plugincmd.go` is the plugin/marketplace CLI; `retrieve_run.go`
> and `imagegen_run.go` build two injected tool runners; `agents.go` loads
> AGENTS.md guidance; `task_status.go` formats background-task status + promotes
> task transcripts; the `smoke_hooks*` trio gates test-only PTY smoke
> entrypoints behind the `smoke` build tag.
...
- **Key symbols:**
  - `main()` — loads `~/.eigen/.env` + config, may re-exec to honor
    `config.theme`/`nerd_font`, resolves the daemon instance, parses flags,
    dispatches every subcommand, then runs the app shell, attaches a TUI to a
    daemon session, or builds an in-process agent (with all tools/MCP/LSP/plugins)
    for interactive or headless use.
  ...
  - `runObserveCmd`, `runDream`, `runMemoryCmd`, `runModelsCmd`, `runSkillCmd`,
    ...
    subcommand handlers dispatched from `main`.
  ...
  - `importResume` / `latestEigenSession` / `recentEigenSessions` /
    `newSessionPath` — resume-by-id/keyword/path and session-file bookkeeping.
  ...
  - `installSkill`, `splitSource`, `acquireTelegramLock`, `signalContext`,
    `multiFlag`, `parseVars`, `firstNonEmpty`, `splitNonEmpty`, `expandTilde`,
    `wdOrDot`, `effectiveModel`, `fail`, `printSessions`/`printSkills`/`printTools`,
    `*ConfigPath` helpers, `notifyCmdline` — utility + path/print helpers.
...
- **Role:** The long-lived session host (`eigen daemon`), its control surface
  ...
  Telegram supervisor.
- **Key symbols:**
  - `runDaemon(cfg)` — starts the persistent host: loads `daemon.env`, builds the
    per-session `build` closure (via `buildSession`), sets model-switcher/titler/
    notifier, restores persisted sessions, starts the nightly dreamer + optional
    Telegram supervisor, listens on the Unix socket, handles graceful shutdown.
...
- **Key symbols:**
  ...
  - `sessionDeps` (struct) + `(*sessionDeps).Close()` + `(*sessionDeps).registryRef()`
    — the agent plus resources the caller keeps alive (MCP clients, LSP manager,
    observe logger); `Close` tears them down; `registryRef` lazily resolves the
    tool registry for `search_tools`.
...
- **Key symbols:**
  ...
  - `guiSuggester()` / `guiSuggestProvider()` — adapt a suggestion model
    (`EIGEN_SUGGEST_MODEL`, else glm-5.1, else the small model) into a
    `feed.Suggester`.
...
- **Role:** Build the injected `retrieve` tool runner (per-project semantic +
  lexical index, opened lazily, synced incrementally).
- **Key symbols:**
...
`runRemoteSession`, `runAttachSock`/`extractSockFlag`, `humanCount`/`humanBytes`/
`shortHash` (daemon stats), `credentialEnvKeys`, `attemptSpan`,
`(*sessionDeps).registryRef`/`Close`, the Wails-bound bridge wiring, and the
build-tagged `runTestSmokeCommand`/`smokeProvider` (smoke tests). See the
structured `deadCode` list for the only genuine (low-confidence) suspects: a few
`sessionDeps` fields written by `buildSession` but never read back.
