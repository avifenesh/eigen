...
### internal/llm/fallback.go (+ process-wide provider freeze)
- **Role:** quota-failover wrapper + a PROCESS-WIDE provider-freeze registry. A quota/billing 429 (e.g. a drained GLM account whose KEY is still present) freezes that provider EVERYWHERE until next local midnight — `FreezeProvider(name)`/`providerFrozen(name)` (mutex-guarded map, auto-expiring). `fallbackProvider.frozen()` also consults it, and `subpolicy.modelCredentialed` skips frozen providers, so a 429 from ANY instance drops the provider from every capability ladder + every fresh provider (no per-instance re-429). `clearFrozenProviders()` is test-only.
