...
- **Used by / entrypoint:** registered as `bash_output`/`kill_shell` in `build.go:199` and `main.go:757-758`.
...
- **Used by / entrypoint:** registered as `diff` in `build.go:196,318` and `main.go:750`. `isGitRepo` also gates the `diff` Definition's `Disabled` flag.
...
- **Used by / entrypoint:** registered as `edit` in `build.go:197,319` and `main.go:752`.

### internal/tool/fsutil.go
- **Role:** Shared low-level helpers for the slice: atomic file write and a ripgrep runner.
- **Key symbols:**
  - `atomicWrite(path, data)` — write to a temp file in the same dir, then `Rename`, so a reader never sees a partial file; `MkdirAll` parents first. Preserves the destination's permission bits when overwriting an existing file (so e.g. a 0o755 script keeps its executable bit; `CreateTemp`'s 0o600 would otherwise strip it), defaulting to 0o644 for new files.
...
- **Used by / entrypoint:** registered as `glob` in `build.go:195,317` and `main.go:746`.
...
- **Used by / entrypoint:** registered as `list` in `build.go:195,317` and `main.go:745`.

### internal/tool/move.go
- **Role:** The `move` tool — move/rename a file or directory; both endpoints fenced by the policy. Falls back to copy-then-remove when `os.Rename` crosses a filesystem boundary (an `/add-dir`'d root may live on a different mount).
- **Key symbols:**
  - `Move(policy) Definition` — mutating; resolves `from`+`to`, verifies the source exists, `MkdirAll`s the destination parent, then `os.Rename`. On an `EXDEV` ("invalid cross-device link") error it falls back to `moveCrossDevice`; any other rename error is surfaced.
  - `moveCrossDevice(src, dst)` — emulates a cross-filesystem rename: `copyPath` then `os.RemoveAll(src)`. On any copy failure it `RemoveAll`s the partial destination so a failed move never leaves a half-written tree while still holding the original.
  - `copyPath(src, dst, fi)` — dispatch by type: directory → `copyDir`, symlink → `os.Symlink` of the read target, regular file → `copyFile`.
  - `copyFile(src, dst, fi)` — copy a regular file, preserving permission bits (explicit `Chmod` after `OpenFile`, which honors umask).
  - `copyDir(src, dst, fi)` — recursive tree copy, `MkdirAll` with the source's mode then `copyPath` each child, reasserting the dir mode at the end.
- **Depends on:** `Policy.Resolve` (`policy.go`). Stdlib `os`, `io`, `path/filepath`, `syscall` (`EXDEV`), `errors`.
- **Used by / entrypoint:** registered as `move` in `build.go:197,320` and `main.go:755`.
...
- **Depends on:** `Policy.Resolve` (`policy.go`); `atomicWrite` (`fsutil.go`). Stdlib `os`, `strings`.
- **Used by / entrypoint:** registered as `multiedit` in `build.go:197,319` and `main.go:753`.
...
- **Key symbols:**
  ...
  - `filePatch.creating()` / `.deleting()` / `.renaming()` — classify a file section by `/dev/null` sentinels / differing paths. `.renaming()` gates the rename read-from-source path in `applyPatch`.
  - `parsePatch(patch)` — dispatches to `parseAgentPatch` for `*** Begin Patch`, else parses standard unified-diff `---/+++/@@/ +-` lines.
  - `parseAgentPatch(lines)` — parses the Codex/OpenAI envelope (`*** Update/Add/Delete File`, `*** Move/Rename to`), lowering it into the same `filePatch` shape; collects `@@` header text as hunk `anchors`.
  - `patchPath`, `parseHunkStart` — strip `a/`,`b/` + tab-timestamp from header paths; read the old start line from `@@ -l,s +l,s @@`.
  - `applyPatch(policy, files)` — computes every file's new content first (reading a rename's content from its `oldPath` source, since `newPath` does not exist yet), writes only if all hunks apply (atomicWrite / os.Remove), then performs rename-deletes for moved files.
  - `applyHunks(content, hunks)` — locate each hunk via `findHunk`, splice new lines in.
  - `findHunk`, `findBlockMatches`, `anchorScore`, `closerToHint`, `clampIndex` — context-match machinery: `findBlockMatches` finds all candidate offsets, `anchorScore` scores by anchors found above the offset, `closerToHint` breaks ties by proximity to the line-number hint.
- **Depends on:** `Policy.Resolve` (`policy.go`); `atomicWrite` (`fsutil.go`). Stdlib `os`, `strings`, `fmt`.
- **Used by / entrypoint:** registered as `apply_patch` in `build.go:197,320` and `main.go:754`.
...
- **Used by / entrypoint:** registered as `read` in `build.go:195,317` and `main.go:744`.
...
- **Used by / entrypoint:** registered as `tree` in `build.go:196,318` and `main.go:749`.
...
- **Used by / entrypoint:** registered as `write` in `build.go:196,319` and `main.go:751`.
