...
| `Classify` | internal/llm/classify.go | Exported, only Go callers are classify_test.go; doc says "legacy deterministic classifier retained for tests". Production routing uses a small-model assessor. | llm-routing |
| `IsFrontend` | internal/llm/classify.go | Exported, only callers are classify_test.go; production frontend signal comes from the LLM assessor's JSON field. Part of the retained legacy classifier. | llm-routing |
