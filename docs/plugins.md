...
| Agents | `agents/*.md` or manifest `agents` path | `~/.eigen/agents/<plugin>-agent-<n>.md` + task role | native foreground/background `task` roles; read-only roles may join `task_group` |
...
Installed plugin agents can be used with the `task` tool by setting `role` to the installed role name (for example `next-task-agent-task-discoverer`). Enabled role names, descriptions, and routing metadata are injected into the agent's system prompt when `task` or `task_group` is available, so the orchestrator does not have to guess role names. The app and chat command palettes surface installed agent roles too: the app jumps to the owning plugin detail, while the chat palette pre-fills a task-role prompt. Agent frontmatter may provide routing metadata (`kind`, `difficulty`, `model`) and read-only tool metadata (`tools` / `allowed-tools`, `read_only`). Agents without read-only metadata inherit the normal task toolset and approval gates. Agents with a verified read-only allowlist (`read`, `grep`, `glob`, `list`/`ls`, `tree`, `symbols`, `diff`) can also be used in `task_group`; mutating/unknown plugin agents stay blocked from parallel fan-out. Legacy installs that adapted agents into generated skills still load as task roles, but new installs use the native agents directory.
